...
// If dso.Compress is true and DSOMsg is compressible then compression is taken into account.
//
...
// Validate checks that DSOMsg, including its TLVs, is valid when composed on server (server = true)
// or client (server = false). Optionally, the request message can be passed to verify that DSOMsg
// is a valid response.
//
